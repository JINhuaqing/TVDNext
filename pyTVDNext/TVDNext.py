...
class TVDNext:
    def __init__(self, rData, dataType=None, saveDir=None, showProgress=True, **paras):
        """
        Input:
            rData: The data matrix, d x n
    ...
    # Data preprocessing, including detrend and decimate, and FFT to obtain tensor X and Y
    def _Preprocess(self):
        ...
        if decimateRate is not None:
            rData = decimate(rData, q=decimateRate)
        ...
        tenData = mat2Tensor(rData, h=self.paras.ffth, stepsize=self.paras.ss)
        self.Y = tenData.Y
        self.X = tenData.X
            

        d, dF, dT = self.Y.shape
        acTime = dT / self.paras.freq
        self.ptime = np.linspace(0, acTime, dT) 
        self.times = np.linspace(0, self.paras.T, dT)
        self.freqs = np.linspace(self.paras.freq/dF, self.paras.freq, dF)
    
    def GetAmat(self):
        if self.Y is None:
            self._Preprocess()
        downRates = self.paras.downRates
        self.Amat = GetAmatTorch(self.Y, self.X, self.times, self.freqs, downRates, self.paras.kerhs)
